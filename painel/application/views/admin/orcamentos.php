...
<script>
   function interna(id){
      window.location.replace("<?= base_url('admin/orcamento_detalhes/') ?>"+id);
   }
   ...
   function excluir(id){
      if(confirm('Tem certeza que deseja excluir esse pedido de orçamento?')){
        $.ajax({
          type:"GET",
          url: "<?= base_url('admin/deletarOrcamento/') ?>"+id,
          async: false,
   ...
   function exibirMensagens(){
      $.ajax({
         data: '',
         type: "GET",
         url: "<?= base_url('admin/ajaxOrcamentos') ?>",
         async: true,
